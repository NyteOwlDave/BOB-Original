...
/*
	...
	Shadow(ray, hit, tmax, color, level, lptr, inside)
...
int Shadow(Ray* ray, Isect* hit, Flt tmax, Color color, int level, Light* lptr, int inside) {
	...
	// check cache first
	if (cached = lptr->light_obj_cache[level]) {
	...
	lptr->light_obj_cache[level] = NULL;    // missed!
		...
		if (in_surf) {           // if exiting
			...
			if (color[0]<rayeps && color[1]<rayeps && color[2]<rayeps) {
				if (tmp_surf.flags & S_CACHE) {  // only if cacheable
					lptr->light_obj_cache[level] = hit->isect_prim;
				}
...
/*
	sShadow(ray, hit, tmax, color, level, lptr, radius, inside)

	ray				ray
	hit				intersection
	tmax			dist to light we are trying to hit
	color			for colored shadows
	level			current tree level
	lptr			light source
	inside			whether inside shape
...
int sShadow(Ray* ray, Isect* hit, Flt tmax, Color color, int level, Light* lptr, int inside) {
	...
	for (sample=0; sample<lptr->samples; sample++) {
		...
		if (Shadow(&tmp_ray, hit, tmax, tmp_color, level, lptr, inside)) {
			VecAdd(tmp_color, color, color);
			visible = 1;
		}
	}

	VecS((1.0/lptr->samples), color, color);
