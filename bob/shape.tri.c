...
void TriNormal(Object* obj, Isect* hit, Point P, Vec N) {
...
/*
	CheckTri() -- 
		make sure that all the vertex normals are on the
...
void ScaleMatrix(Flt scale, Vec in[3], Vec out[3]) {

	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			out[i][j] = in[i][j] * scale;
		}
	}
}
