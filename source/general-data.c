...
int 			Yresolution = 240;				// screen height

// Session
int				start_line = 0;					// session start line
int				stop_line = 0;				    // session stop line
BOOL		    resume = FALSE;	        		// resume prior session
BOOL            console = FALSE;                // enter console mode on error
...
// Studio (System)
BOOL			jitter = FALSE;					// none
int				bunching = BUNCHINGFACTOR;		// composite bunch size
int				antialias = A_NONE;			    // default to none
int				adapt_dist = ADAPT_DIST;		// threshold for adaptive sampling
...
BOOL			no_shadows = FALSE;				// shadows by default
BOOL			caustics = FALSE;		        // no fake caustics
BOOL			exp_trans = TRUE;				// enable exponential transparency
BOOL			fuzzy_ray = FALSE;              // Ray fuzziness
