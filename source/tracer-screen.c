...
static void Scan2(void)
{
	...
	*/
    ...
    for (y = start_line; y < stop_line; y += 6)
    {
        ...
        for (x = 0; x < x_res; x += 6)
        {
            ...
            if (comp(buf[j][i].r, buf[j - 3][i - 3].r) &&
                ...
                comp(buf[j][i].b, buf[j - 3][i - 3].b))
            { // close enough so fill
                if (!flags[j - 1][i])
                {
            ...
            { // else have to calc upper-left quad
                for (i = x; i < x + 4; i++)
                {
                    for (j = 0; j < 4; j++)
                    {
            ...
            { // else have to calc upper-right quad
                for (i = x + 3; i < x + 7; i++)
                {
                    for (j = 0; j < 4; j++)
                    {
            ...
            { // else have to calc lower-left quad
                for (i = x; i < x + 4; i++)
                {
                    for (j = 3; j < 7; j++)
                    {
            ...
            { // else have to calc lower-right quad
                for (i = x + 3; i < x + 7; i++)
                {
                    for (j = 3; j < 7; j++)
                    {
    ...
    // Release pixel and flag buffers
...
static void Scan3(void)
{
    ...
    for (int y = start_line; y < stop_line; y++)
    {
        ...
        for (int x = 0; x < x_res; x++)
        {
            ...
            { // buff to top row of win
                if (win[i][0][3] = buff[3][x * SIDE + i])
                { // if cooked
            ...
            for (int i = 0; i < SIDE + 1; i++) // bottom row of win to buff
                if (buff[3][x * SIDE + i] = win[i][SIDE][3])
                { // if cooked
            ...
            for (int j = 0; j < SIDE + 1; j++)
            { // right edge of win to left
                if (win[0][j][3] = win[SIDE][j][3])
                { // if cooked
