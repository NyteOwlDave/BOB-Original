...
static void yy_studio(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        case THRESHOLD:
            ...
            break; // int
        case RESOLUTION:
            xx_studio_resolution();
            break; // int
        case JITTER:
            xx_studio_jitter();
            break; // flag
        case NO_SHADOWS:
            xx_studio_no_shadows();
            break; // flag
        case CAUSTICS:
            xx_studio_caustics();
            break; // flag
        case NO_EXP_TRANS:
            xx_studio_no_exp_trans();
            break; // flag
        case PROJECTION:
            xx_studio_projection();
            break; // keyword
        case ANTIALIAS:
            xx_studio_antialias();
            break; // keyword
        case BKG:
            xx_studio_background();
            break; // block
        default:
            // Syntax error
...
static Surface *yy_surface(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        {
        case AMBIENT:
            xx_surface_ambient(surf);
            break; // vec or block
        case DIFFUSE:
            xx_surface_diffuse(surf);
            break; // vec or block
        case SPECULAR:
            xx_surface_spec(surf);
            break; // vec or block
        case TRANSPARENT:
            xx_surface_trans(surf);
            break; // vec or block
        case FUZZ:
            xx_surface_fuzz(surf);
            break; // float
        case IOR:
            xx_surface_ior(surf);
            break; // float
        case SHINE:
            xx_surface_shine(surf);
            break; // float (plus optional vec)
        case NO_ANTIALIAS:
            xx_surface_no_antialias(surf);
            break; // flag
        case TEXTURE:
            xx_surface_texture(surf);
            break; // block
        case BUMP:
            xx_surface_bump(surf);
            break; // block
        default:
...
static void xx_texture_pattern(Texture *tex)
{
    ...
    {
    case CHECKER:
        tex->func = tex_checker;
        break;
    case SPHERICAL:
        tex->func = tex_spherical;
        break;
    case NOISE:
        tex->func = tex_noise;
        break;
...
static Texture *xx_texture(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        {
        case PATTERN:
            xx_texture_pattern(tex);
            break; // keyword
        case SCALE:
            xx_texture_scale(tex);
            break;      // vec or float
        case FROM:      // Fall through
        case TRANSLATE: // Fall through
        case OFFSET:    // FAll through
        case CENTER:
            xx_texture_translate(tex);
            break; // vec
        case BLEND:
            xx_texture_blend(tex);
            break; // float
        case FUZZ:
            xx_texture_fuzz(tex);
            break; // float
        case RADIUS:
            xx_texture_radius(tex);
            break; // float
        case TERMS:
            xx_texture_terms(tex);
            break; // int
        case SURFACE:
            xx_texture_surface(tex);
            break; // block
        case TURBULENCE:
            xx_texture_turbulence(tex);
            break; // block
        case WAVE:
            xx_texture_wave(tex);
            break; // block
...
static Bump *xx_bump(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        case OFFSET:    // FAll through
        case CENTER:
            xx_bump_translate(bump);
            break; // vec
        case TURBULENCE:
            xx_bump_turbulence(bump);
            break; // block
        case WAVE:
            xx_bump_wave(bump);
            break; // block
...
static Turbulence *xx_turbulence(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        switch (cur_token)
        {
            ...
            // terms amp scale translate
        case TERMS:
            cur_turb->terms = read_int();
            break;
        case AMP:
            cur_turb->amp = read_float();
            break;
        case SCALE:
            read_vec(cur_turb->scale);
            break;
        case FROM:      // Fall through
        case TRANSLATE: // Fall through
        case OFFSET:    // FAll through
        case CENTER:
            read_vec(cur_turb->trans);
            break;
        default:
...
static Wave *xx_wave(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        case CENTER:
            ...
            break; // vec
        case AMP:
            xx_wave_amplitude(wave);
            break; // float
        case DAMPING:
            xx_wave_damping(wave);
            break; // float
        case PHASE:
            xx_wave_phase(wave);
            break; // float
        case LENGTH:
            xx_wave_wavelength(wave);
            break; // float
        default:
...
static Texmap *xx_texmap(void)
{
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        {
        case ACROSS:
            xx_texmap_across(texmap);
            break;      // vec
        case FROM:      // Fall through
        case TRANSLATE: // Fall through
        case OFFSET:    // FAll through
        case CENTER:
            xx_texmap_center(texmap);
            break; // vec
        case NORMAL:
            xx_texmap_normal(texmap);
            break; // vec
        case SCALE:
            xx_texmap_scale(texmap);
            break; // float
        case IMAGE:
            xx_texmap_image(texmap);
            break; // filename
        default:
...
static void xx_clip_center(Clip *clip)
{
    ...
    if (TransTop)
    {
        trans_vector(TransTop->mat, clip->center, clip->center);
    }
    if (!(clip->type & C_PLANE))
    {
        clip->type |= C_SPHERE;
    }
}
...
static void xx_clip_normal(Clip *clip)
{
    ...
    if (TransTop)
    {
        trans_normal(TransTop->mat, clip->normal, clip->normal);
    }
    cc_clip_plane(clip);
}
...
static void xx_clip_apex(Clip *clip)
{
    ...
    if (TransTop)
    {
        trans_vector(TransTop->mat, clip->apex, clip->apex);
    }
    cc_clip_cone(clip);
}
...
static void xx_clip_base(Clip *clip)
{
    ...
    if (TransTop)
    {
        trans_vector(TransTop->mat, clip->base, clip->base);
    }
    cc_clip_cone(clip);
}
...
#pragma endregion
    ...
    while (get_token() != RIGHT_BRACE)
    {
        ...
        case NORMAL:
            ...
            break; // vec
        case APEX_RADIUS:
            xx_clip_apex_radius(clip);
            break; // float
        case BASE_RADIUS:
            xx_clip_base_radius(clip);
            break; // float
        case RADIUS:
            xx_clip_radius(clip);
            break; // float
